...
export class AuthService {
        ...
        tap({
          next: (response: AuthResponse) => {
            ...
            if (response && response.userId) {
              // Extract user data from the auth response and auto-login
              const user: User = {
                id: response.userId,
        ...
        tap({
          next: (response: AuthResponse) => {
            ...
            if (response && response.userId) {
              ...
              console.log(
                'Login successful - user stored, cookies set by server:',
                {
                  user,
                  tokenType: response.tokenType,
                  isAuthenticated: this._isAuthenticated(),
  ...
  checkAuthStatus() {
    ...
    return this.http
      .get<AuthResponse>(`${this.apiUrl}/check`, {
        withCredentials: true,
      })
      .pipe(
        tap({
          next: (response: AuthResponse) => {
            console.log(
              `Auth check successful on ${isServer ? 'server' : 'client'}:`,
              response
            );
            if (response && response.userId) {
              const user: User = {
          ...
          error: (err) => {
            console.log(
              `Auth check failed on ${isServer ? 'server' : 'client'}:`,
              err
            );
            this._user.set(null);
            this._isAuthenticated.set(false);
